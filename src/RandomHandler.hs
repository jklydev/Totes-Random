...
getTweet :: String -> String
getTweet url = "https://publish.twitter.com/oembed?url="++ url++"&hide_media=true"
...
randVal = do
  ...
  let requests = map getTweet tweets
  ...
  return $ decodeUtf8 $ encode $ toJSON (TwitterSeed randOfTheKing requests)
